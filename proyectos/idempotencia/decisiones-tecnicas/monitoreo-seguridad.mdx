...
**Acceso:** panel de Portainer vía Tailscale{/* (`http://100.99.206.50:9000`) */} (requiere Tailscale activo). Nota: el setup inicial de usuario admin tiene una ventana de ~5 minutos; si expira, se necesita el `setup_token` (`docker logs portainer | grep setup_token`) junto con usuario/contraseña.
